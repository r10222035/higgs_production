...
\label{sec:data_augmentation}
    To improve the training performance, we consider the various data augmentation methods.

    \subsection{\texorpdfstring{$p_{\mathrm{T}}$}{pT} smearing}% (fold)
    \label{sub:pt_smearing}
        The $p_{\text{T}}$ smearing method is used to simulate detector resolution effects on the transverse momentum of event constituents. This method resamples the transverse momentum $p_{\text{T}}$ of event constituents according to the normal distribution:
            \begin{equation}
        ...
        \begin{table}[htpb]
            \centering
            \caption{The CNN training results with different augmentation size. The ACC and AUC are evaluated based on 10 training. The selection cuts of the number of gluon jets are applied.}
